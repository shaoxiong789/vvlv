...
var VirtualList = /** @class */function (_super) {
    ...
    VirtualList.prototype.mounted = function () {
        ...
        this.list$.next(this.list);
        // 数据加载完毕后，对快照查漏补缺
        this.subscription.add(this.list$.pipe(pairwise(), tap(function (_a) {
            var newList = _a[1];
        ...
        // window resize
        this.subscription.add(fromEvent(window, 'resize').pipe(withLatestFrom(options$), map(function (_a) {
            var options = _a[1];
        ...
        // 计算可滚动内容的高度
        var scrollHeight$ = combineLatest(this.list$, options$).pipe(map(function (_a) {
            var list = _a[0],
                height = _a[1].height;
            return list.length * height;
        }));
        // 滚动触发加载
        var scrolling$ = combineLatest(scrollTop$, scrollDirection$).pipe(map(function (_a) {
            var scrollTop = _a[0],
                dir = _a[1];
        ...
        var pullUpLoad$ = defer(function () {
            ...
            return combineLatest(scrolling$, _this.containerHeight$, scrollHeight$).pipe(map(function (_a) {
                var _b = _a[0],
                    scrollTop = _b[0],
                    dir = _b[1],
                    ch = _a[1],
        ...
        var shouldUpdate$ = combineLatest(scrollWin$.pipe(map(function (scrollTop) {
            return scrollTop;
        ...
        // 计算当前需要的数据区块
        var dataInViewSlice$ = combineLatest(this.list$, options$, shouldUpdate$).pipe(withLatestFrom(scrollDirection$, actualRows$), map(function (_a) {
            var _b = _a[0],
                list = _b[0],
                height = _b[1].height,
                _c = _b[2],
        ...
        this.subscription.add(combineLatest(dataInViewSlice$, scrollHeight$).subscribe(function (_a) {
            var list = _a[0],
                scrollHeight = _a[1];
            _this.viewlist = list;
    ...
    VirtualList = __decorate([Component({})], VirtualList);
    return VirtualList;
}(Vue);
